...
#     std_dev = get_value(inversion_prob_var, 0.01)
...
#     population = Population(variables_count, population_size, precision, start_, end_, func, "max" if is_maximization else "min", std_dev, alpha, beta, representation_type="binary" )
...
# alpha = tk.StringVar(value=0.01)
# beta = tk.StringVar(value=0.01)
...
# mutation_method_var = tk.StringVar(value="Uniform")
...
root = tk.Tk()
root.title("Genetic Algorithm Configuration")
root.geometry("350x580") # Zwiększono wysokość okna, aby pomieścić wszystkie pola
...
mutation_method_var = tk.StringVar(value="Boundary") # Domyślna wartość
...
fields = [
    ...
    ("Percentage elite strategy", elite_percent_var),
    ("Count elite strategy", elite_count_var),
    ("Alpha", alpha_var), # Dodano pole dla alpha
    ("Beta", beta_var),   # Dodano pole dla beta
    ("Mutation probability", mutation_prob_var),
    ("Inversion probability", inversion_prob_var),
]
...
tk.Label(select_frame2, text="Cross method", anchor="w", width=25).grid(row=0, column=0)
cross_method_combobox = ttk.Combobox(select_frame2, textvariable=cross_method_var, values=["One-Point", "Two-Point", "Uniform crossover", "Granular crossover", "arithmetic_crossover_real", "linear_crossover_real", "blend_crossover_alpha_real", "blend_crossover_alpha_beta_real", "average_crossover_real"], width=15)
cross_method_combobox.grid(row=0, column=1)
cross_method_combobox.state(["readonly"])
...
tk.Label(select_frame, text="Mutation method", anchor="w", width=25).grid(row=1, column=0)
mutation_method_combobox = ttk.Combobox(select_frame, textvariable=mutation_method_var, values=["Boundary", "One-Point", "Two-Point", "Gaussian", "Uniform"], width=15) # Dodano Gaussian i Uniform
mutation_method_combobox.grid(row=1, column=1)
mutation_method_combobox.state(["readonly"])
